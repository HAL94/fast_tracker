...
class JournalService(BaseService):
    ...
    async def _process_worklogs(self, user_id: UUID, tasks: list[UpsertActivityTask]) -> None:
    ...
    async def batch_worklog(self, data: TaskBatchDto, user_id: UUID) -> List[WorklogBase]:
        ...
        upsert_result, affected_dates = await self._process_worklogs(user_id, data.tasks)
